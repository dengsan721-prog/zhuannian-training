...
export const DEMO_MODE_KEY = 'zhuannian:demo-mode';

const demoUserId = '00000000-0000-4000-8000-000000000101';
const demoSessionId = '40000000-0000-4000-8000-000000000001';
const demoCompletionId = '60000000-0000-4000-8000-000000000001';

const demoScene: PublishedSceneVersion = {
  schemaVersion: 1,
  sceneCode: 'PC-003',
  ...
  thoughtOptions: [
    {
      ...
      label: '他根本没把我的话当回事',
      likelyEmotion: '愤怒',
      likelyWords: '你说话从来不算数',
      likelyAction: '立即夺走手机',
      possibleResponse: '孩子可能争抢或躲开',
    },
    {
      id: 'ungrateful',
      label: '他一点都不理解我的辛苦',
      likelyEmotion: '委屈',
      likelyWords: '我做这一切都是为了谁',
      likelyAction: '逐一翻旧账',
      possibleResponse: '孩子可能防御或沉默',
    },
    {
      id: 'loss-control',
      label: '我已经管不住他了',
      likelyEmotion: '焦虑',
      likelyWords: '再这样下去你就完了',
      likelyAction: '升级惩罚',
      possibleResponse: '孩子可能隐藏使用行为',
    },
  ],
  ...
  hypotheses: [
    {
      id: 'need-autonomy',
      kind: 'benevolent',
      text: '他也许在保护一天里少有的自主时间，需要通过询问确认。',
      evidencePrompt: '他是否在被频繁催促后更难停下？',
    },
    {
      id: 'transition-skill',
      kind: 'constraint',
      text: '他可能缺少从娱乐切换到任务的能力，而不只是故意违约。',
      evidencePrompt: '提前提醒和共同设定结束动作是否有帮助？',
    },
    {
      id: 'rule-boundary',
      kind: 'boundary',
      text: '反复违背共同约定仍需要清楚、稳定且可执行的规则。',
      evidencePrompt: '规则是否提前讲清并由双方确认？',
    },
  ],
  evidenceChecks: [
...
const sceneRepository: SceneRepository = {
  ...
  async getBySlug(slug) {
    return slug === demoScene.slug ? demoScene : null;
  },
  async getPublishedById(sceneVersionId) {
    return sceneVersionId === demoScene.id ? demoScene : null;
  },
};

const runtimeRepository: TrainingRuntimeRepository = {
  async startTraining() {
    return { sessionId: demoSessionId, route: 'continue' };
  },
...
const progressRepository: ProgressRepository = {
  async complete() {
    const awarded = !demoCompleted;
    demoCompleted = true;
    return {
      completionId: demoCompletionId,
      awarded,
      pointsDelta: awarded ? 10 : 0,
    };
  },
  async saveReview() {
    return {
  ...
  async getPrivateProgress() {
    return {
      points: demoCompleted ? 10 : 0,
      completedScenes: demoCompleted ? 1 : 0,
      reviewsCompleted: 0,
      thisWeekCompletions: demoCompleted ? 1 : 0,
      badges: [],
      unlockedSurprises: [],
      classAggregate: null,
    };
...
const supportRepository: SupportRepository = {
  async createSupportTicket() {
    throw new Error('demo_mode_does_not_submit');
  },
  async listMySupportTickets() {
    return [];
  },
  async revokeSupportConsent() {
    throw new Error('demo_mode_does_not_submit');
  },
  async stopTrainingForSafety(sessionId) {
    return { sessionId, route: 'safety-stop' };
  },
  async createSafetyReport() {
    throw new Error('demo_mode_does_not_submit');
  },
  async listMySafetyReports() {
    return [];
  },
};
...
export const demoEnvironment = {
  ...
  progressRepository,
  supportRepository,
  getCurrentUserId: async () => demoUserId,
};
